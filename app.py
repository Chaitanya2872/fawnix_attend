...
# ✨ Public (unauthenticated) aggregate stats for marketing pages
from routes.public_stats import public_stats_bp
...
app.register_blueprint(public_stats_bp, url_prefix="/api/public")
...
def _frontend_dist_exists() -> bool:
    return os.path.isdir(FRONTEND_DIST) and os.path.isfile(
...
def auto_clockout_job():
    ...
    try:
        ...
        if result["success"]:
            ...
            if result.get("details"):
                for detail in result["details"]:
                    logger.info(
                        f"   • {detail['employee_name']} ({detail['employee_email']})"
                    )
                    logger.info(f"     Working hours: {detail['working_hours']}h")
                    logger.info(
                        f"     Activities closed: {detail['activities_closed']}"
                    )
                    logger.info(
                        f"     Field visits closed: {detail['field_visits_closed']}"
                    )
        else:
        ...
        logger.error("=" * 80)
        return {"success": False, "message": str(e), "auto_clocked_out": 0}
...
def _parse_positive_int(raw_value, default_value, label):
    ...
    try:
        ...
        logger.warning(
            "%s='%s' is invalid. Falling back to %s.", label, raw_value, default_value
        )
...
def _parse_clock_times(raw_times: str):
    ...
    for raw_value in raw_times.split(","):
        ...
        if hour < 0 or hour > 23 or minute < 0 or minute > 59:
            raise ValueError(
                f"Invalid time '{value}'. Hour must be 00-23 and minute must be 00-59."
            )
...
def get_auto_clockout_schedule_config():
    ...
    schedule_mode = (
        os.environ.get("AUTO_CLOCKOUT_SCHEDULE_MODE", "production").strip().lower()
    )
    ...
    coalesce = _env_to_bool(
        os.environ.get("AUTO_CLOCKOUT_COALESCE", "true"), default=True
    )
    enforce_single_process = _env_to_bool(
        os.environ.get("AUTO_CLOCKOUT_ENFORCE_SINGLE_PROCESS", "true"), default=True
    )
    lock_file = os.environ.get(
        "AUTO_CLOCKOUT_LOCK_FILE", _get_default_scheduler_lock_file()
    )

    selected_raw_times = (
        production_times if schedule_mode == "production" else testing_times
    )
    ...
    return {
        ...
        "active_times_text": ", ".join(
            f"{hour:02d}:{minute:02d}" for hour, minute in active_times
        ),
            ...
            jsonify(
                {
                    ...
                    "auto_clockout": {
                        ...
                        "misfire_grace_seconds": schedule_config[
        ...
        jsonify(
            {
                ...
                "endpoints": {
                    ...
                    "leads": {
                        ...
                        "PATCH /api/leads/{id}/remarks/{remarkId}": "Edit a remark version on a lead",
                        "POST /api/leads/{id}/quotations": "Create a quotation for this lead",
                        "GET /api/leads/{id}/quotations": "List quotations for this lead",
                        "GET /api/leads/{id}/quotations/{quotationId}": "Get a quotation's detail",
                        "PATCH /api/leads/{id}/quotations/{quotationId}": "Update a quotation's items/status",
                    },
                    "admin": {
                ...
                "new_features_v2": {
                    ...
                    "activity_approvals": {
                        ...
                        "endpoints": [
                            "/api/approvals/late-arrival/request",
...
def after_request(response):
    ...
    response.headers["X-Features"] = (
        "refresh-tokens,location-reports,distance-monitoring,approvals,lead-management,auto-clockout"
    )
    return response
