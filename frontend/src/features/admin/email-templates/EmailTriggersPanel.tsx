...
type EmailTrigger = { id?: number | string; trigger_key: string; trigger_name: string; template_key: string; trigger_type: TriggerType; event_name?: string | null; schedule_cron?: string | null; to_recipients: string[]; cc_recipients: string[]; bcc_recipients: string[]; variables: Record<string, unknown>; active: boolean; next_run_at?: string | null; last_run_at?: string | null; last_status?: string | null; last_error?: string | null }
...
const blank: EmailTrigger = { trigger_key: '', trigger_name: '', template_key: '', trigger_type: 'event', event_name: '', schedule_cron: '0 9 * * 1-5', to_recipients: [], cc_recipients: [], bcc_recipients: [], variables: {}, active: true }

export default function EmailTriggersPanel({ apiRequest, templateKeys }: Props) {
  const [items, setItems] = useState<EmailTrigger[]>([]); const [events, setEvents] = useState<EmailEvent[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('')
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { const [t, e] = await Promise.all([apiRequestRef.current(`${endpoint}/triggers`), apiRequestRef.current(`${endpoint}/events`)]); setItems(Array.isArray(t?.data) ? t.data : []); setEvents(Array.isArray(e?.data) ? e.data : []) } catch (err) { setError(errorText(err)) } finally { setLoading(false) } }, [])
  ...
  const when = (item: EmailTrigger) => item.trigger_type === 'event' ? <code className="et-key">{item.event_name}</code> : item.trigger_type === 'schedule' ? <><code className="et-key">{item.schedule_cron}</code>{item.next_run_at && <div className="adm-cell-secondary">next {new Date(item.next_run_at).toLocaleString()}</div>}</> : <span className="adm-cell-secondary">On demand</span>
  return <section className="adm-table-card table-card">
    ...
    {editing && createPortal(<TriggerEditor trigger={editing} events={events} templateKeys={templateKeys} apiRequest={apiRequest} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void load() }}/>, document.body)}
    {running && createPortal(<RunDialog trigger={running} apiRequest={apiRequest} onClose={() => { setRunning(null); void load() }}/>, document.body)}
  </section>
}

function TriggerEditor({ trigger, events, templateKeys, apiRequest, onClose, onSaved }: { trigger: EmailTrigger; events: EmailEvent[]; templateKeys: string[]; apiRequest: ApiRequest; onClose: () => void; onSaved: () => void }) {
  ...
  const selectedEvent = events.find(e => e.eventName === form.event_name)
  const save = async () => {
  ...
  return <div className="et-overlay" role="dialog" aria-modal="true" aria-label="Email trigger editor"><section className="et-drawer" ref={dialogRef}><header className="et-drawer-head"><div><p className="et-eyebrow">Trigger setup</p><h2>{trigger.id ? 'Edit trigger' : 'New trigger'}</h2></div><button className="et-close" onClick={onClose} aria-label="Close">×</button></header>{error && <div className="et-notice" role="alert">{error}</div>}
    ...
    </form><aside className="et-preview"><div><p className="et-eyebrow">Available variables</p><h3>{form.trigger_type === 'event' ? selectedEvent?.label || 'Event' : form.trigger_type === 'schedule' ? 'Scheduled run' : 'Manual run'}</h3><p>Use these in the template and in recipient fields, e.g. <code>{'{{manager_email}}'}</code>. Blank recipients are skipped.</p></div>
      <div className="et-mail"><pre>{form.trigger_type === 'event' ? (selectedEvent?.variables || []).map(v => `{{${v}}}`).join('\n') : form.trigger_type === 'schedule' ? '{{run_date}}\n+ default variables' : 'Default variables, plus any\nsupplied when you click Run now'}</pre></div></aside></div></section></div>
}
