...
def _calculate_late_by_minutes(reference_time: time, shift_start: time) -> int:
    """Calculate positive lateness in minutes for a given time."""
    reference_dt = datetime.combine(datetime.today(), reference_time)
    shift_dt = datetime.combine(datetime.today(), shift_start)
...
def _format_exception_reason_context(exception_type: str) -> str:
    normalized = (exception_type or '').strip().lower()
    if normalized == 'late_arrival':
        return 'late arrival'
    if normalized == 'early_leave':
...
def _format_exception_expected_time_label(exception_type: str) -> str:
    normalized = (exception_type or '').strip().lower()
    if normalized == 'late_arrival':
        return 'login'
    if normalized == 'early_leave':
    ...
    try:
        ...
        if effective_minutes is not None and expected_time_display:
            whatsapp_template_parameters = [
                ...
                reason_text,
                _format_exception_expected_time_label(exception_type),
...
def request_late_arrival_exception(emp_code: str, reason: str,
    ...
    try:
        ...
        if cursor.fetchone():
            return ({
                "success": False,
                "message": "Late arrival must be submitted before clock-in"
            }, 400)

        # Check if exception already exists for today
        cursor.execute("""
    ...
    try:
        ...
        query = f"""
            SELECT
                ...
                -- login_time is a timestamp. exception_time is TIME in the
                -- current migration, but older databases may have TIMESTAMP;
                -- normalize both forms to TIME before constructing the fallback.
                COALESCE(
                    a.login_time,
