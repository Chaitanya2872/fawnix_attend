...
class PendingAttendanceCursor:
    ...
    def execute(self, sql, params=None):
        ...
        if "SELECT id FROM attendance WHERE employee_email = %s AND date = %s AND status = %s AND logout_time IS NULL ORDER BY created_at DESC, id DESC LIMIT 1" in normalized_sql:
            self.fetchone_value = self.pending_attendance
            return

        if "INSERT INTO attendance (" in normalized_sql and "phone_number" in normalized_sql and "RETURNING id" in normalized_sql:
            assert params[7] == "logged_in"
...
def test_request_late_arrival_keeps_exception_detached_from_attendance(monkeypatch):
...
def test_clock_in_creates_normal_attendance_row_after_standalone_late_arrival_request(monkeypatch):
    connection = PendingAttendanceConnection(pending_attendance=None)
    ...
    assert connection.cursor_obj.updated_attendance_id is None
    assert connection.commit_count == 1
