...
def test_update_lead_status_uses_dedicated_status_endpoint(monkeypatch):
    captured = {}
    ...
    response, status_code = lead_service.update_lead_status(
        "lead-123",
        {"emp_email": "john@example.com"},
        {"status": "CONTACTED", "remark": "Spoke with the client"},
    )
    ...
    assert captured == {
        "method": "PATCH",
        "url": f"{lead_service.CRM_BASE_URL}/api/leads/lead-123/status",
        "json": {"status": "CONTACTED", "remark": "Spoke with the client"},
    }
